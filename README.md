...
- 新增**投资分析独立模块**（顶部「🧠 投资分析」tab），把原「投资分析」项目的单票尽调流水线搬进本工作台，作为独立功能扩展：
  ...
  - **设置区**：LLM 端点/密钥配置（回显打码 + 测试连接）、投资者画像（风险/周期/仓位上限/股息偏好）、运行选项（是否启用 LLM、联网取证、忽略缓存、取数起点、每股分红、文件名后缀）。
  - **单只 / 批量跑**：可填单只股票代码运行，也可粘贴多行 `代码 名称` 批量串行跑（单次最多 50 只）；任务在后台线程执行，前端轮询进度（阶段 + 百分比 + 日志流），不阻塞 Web 服务。
  - **报告**：跑完生成杂志风 HTML 尽调报告，结果页可直接打开查看；批量跑额外给出对比汇总表（综合分 / 建议 / 仓位上限 / 模块来源）。
  - **回退开关**：`config/defaults.json` 的 `invest.enabled` 设为 `false`（或在 `user.json` 覆盖）即不注册蓝图、不渲染 tab，行为与 0.3.26 完全一致，便于「用得不顺手」时一键回退。
- `templates/index.html` 静态资源缓存击穿参数升级到 `?v=0.5.0`。

**v0.3.26**
- 修复「当天」视图下 1 分钟 K 线无数据（非交易日/未开盘/接口异常）时页面只显示「当天 K 线加载失败」的问题：
  - 后端 `/api/chart/intraday/<code>` 在无分时数据时返回 `fallback_date`（上一交易日）。
  - 前端 `loadIntradayChart` 收到回退日期后，自动加载并绘制上一交易日日 K 线图，提示改为「当天 K 线加载失败，加载上一个交易日 YYYY-M-D 日 K 线」。
  - 这样周末或节假日打开「当天」也能看到最近走势，而不是空白/失败。
- `templates/index.html` 静态资源缓存击穿参数升级到 `?v=0.3.26`。
